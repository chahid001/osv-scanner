...
func Command(stdout, stderr io.Writer, r *reporter.Reporter) *cli.Command {
	return &cli.Command{
		...
		Flags: []cli.Flag{
			...
			&cli.BoolFlag{
				Name:  "html",
				Usage: "generate an HTML report (developed by chahid001)",
			},
			&cli.IntFlag{
				Name:  "failOnCVSS",
				Usage: "add CVSS rule for failing (developed by chahid001)",
				Value: -1,
			},
			&cli.StringFlag{
				Name:  "project",
				Usage: "add project name (developed by chahid001)",
				Value: "false",
			},
			&cli.StringFlag{
				Name:  "name",
...
func action(context *cli.Context, stdout, stderr io.Writer) (reporter.Reporter, error) {
	...
	if context.IsSet("L") {
		...
		fmt.Println("Please add -L <pubspec.lock path> to scan")
		return nil, nil
	}
	if context.Bool("html") {

		failOn := context.Int("failOnCVSS")
		project_n := ""
		if context.IsSet("project") {
			projectName := context.String("project")
			project_n = projectName
			if projectName == "" || strings.HasPrefix(projectName, "-") || projectName == "false"{
				fmt.Fprintln(stdout, "Please provide a valid name for the project.")
				return r, nil
			}
		}
		name_rp := ""
		if context.IsSet("name") {
			reportName := context.String("name")
			name_rp = reportName
			if reportName == "" || reportName == "false"{
				fmt.Fprintln(stdout, "Please provide a valid path for the project.")
				return r, nil
			}
		}
		if context.Bool("npm") {
